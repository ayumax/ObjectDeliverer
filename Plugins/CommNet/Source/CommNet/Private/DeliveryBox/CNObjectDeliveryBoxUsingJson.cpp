#include "CNObjectDeliveryBoxUsingJson.h"
#include "Utils/CNObjectUtil.h"
#include "Utils/CNStringUtil.h"
#include "JsonObjectConverter.h"

UCNObjectDeliveryBoxUsingJson::UCNObjectDeliveryBoxUsingJson()
{
...
void UCNObjectDeliveryBoxUsingJson::Send_Implementation(const UObject* message)
{
	...
	TArray<uint8> buffer;
	UCNStringUtil::StringToBuffer(OutputString, buffer);

	RequestSend.ExecuteIfBound(buffer);
}

void UCNObjectDeliveryBoxUsingJson::NotifyReceiveBuffer_Implementation(const TArray<uint8>& buffer)
{
	TSharedRef<TJsonReader<TCHAR>> JsonReader = TJsonReaderFactory<TCHAR>::Create(UCNStringUtil::BufferToString(buffer));
...
TSharedPtr<FJsonObject> UCNObjectDeliveryBoxUsingJson::CreateJsonObject(const UObject* Obj)
{
	TSharedPtr<FJsonObject> JsonObject = MakeShareable(new FJsonObject());
	...
	for (TFieldIterator<UProperty> PropIt(Obj->GetClass(), EFieldIteratorFlags::IncludeSuper); PropIt; ++PropIt)
	{
		...
		switch (UCNObjectUtil::GetUPropertyType(Property))
		{
